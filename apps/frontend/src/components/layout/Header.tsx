...
export const Header: React.FC<{ toggleSidebar?: () => void, isSidebarOpen?: boolean }> = ({ toggleSidebar }) => {
  ...
  const getOptionBtnClass = (isActive: boolean) => 
    `rounded-full px-2.5 py-1 flex items-center gap-1 font-medium transition-all cursor-pointer ${
      isActive 
        ? 'bg-slate-200 text-black shadow-sm dark:bg-slate-600 dark:text-white dark:shadow-none' 
        : 'text-black hover:bg-slate-100 dark:text-slate-400 dark:hover:text-slate-200 bg-transparent'
    }`;
    ...
    <header className="sticky top-0 z-40 bg-surface-main/95 dark:bg-slate-900/95 backdrop-blur-sm border-b border-[#E5E8EE] dark:border-slate-800 h-16 w-full flex items-center justify-between px-4 sm:px-6 transition-colors duration-200">
      <div className="flex items-center gap-3 sm:gap-4">
        <button 
          ...
          className="p-2 rounded-lg text-[#008FD5] bg-[#E6F4FA] dark:text-slate-400 dark:bg-transparent dark:hover:text-slate-300 hover:bg-sky-100 dark:hover:bg-slate-800 focus:outline-none transition-colors cursor-pointer"
      ...
      <div className="flex items-center gap-3 sm:gap-5 relative">
        <div className="relative" ref={notifRef}>
          <button 
            ...
            className="relative p-2 rounded-lg text-[#008FD5] bg-[#E6F4FA] dark:text-slate-400 dark:bg-transparent dark:hover:text-slate-300 hover:bg-sky-100 dark:hover:bg-slate-800 transition-colors cursor-pointer"
          ...
          <div className={`${isNotifOpen ? 'block' : 'hidden'} absolute right-0 mt-2 w-80 sm:w-96 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-[#E5E8EE] dark:border-slate-700 py-2 z-50 transform origin-top-right transition-all`}>
            <div className="px-4 py-3 border-b border-[#E5E8EE] dark:border-slate-700 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <h4 className="text-base font-bold text-slate-900 dark:text-white">{t('header.notifications')}</h4>
              ...
              <button className="text-xs font-semibold text-[#008FD5] hover:text-[#006193] dark:hover:text-sky-300 hover:underline transition-colors cursor-pointer" type="button">{t('header.mark_all_read')}</button>
            </div>
        ...
        <div className="relative" ref={profileRef}>
          ...
          >
            ...
            <div className="hidden sm:flex flex-col text-left leading-none">
              ...
              <span className="text-[11px] text-slate-500 dark:text-slate-400 font-medium mt-0.5">{t('header.student')}</span>
            </div>
          ...
          <div className={`${isProfileOpen ? 'block' : 'hidden'} absolute right-0 mt-2 w-72 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-[#E5E8EE] dark:border-slate-700 py-2 z-50 transform origin-top-right transition-all`}>
            <div className="px-4 py-3 border-b border-[#E5E8EE] dark:border-slate-700">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <span className="inline-block mt-1 px-2 py-0.5 rounded text-[10px] font-semibold bg-[#E6F4FA] dark:bg-sky-950 text-[#006193] dark:text-sky-300">{t('header.official_student')}</span>
                </div>
              </div>
            </div>
            ...
            <div className="py-1">
              ...
              <div className="flex items-center justify-between px-4 py-2.5 text-sm text-black dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors text-left">
                <div className="flex items-center gap-3">
                  <span className="material-symbols-outlined text-[20px] text-slate-400">language</span>
                  <span>{t('header.language')}</span>
                </div>
                <div className="flex items-center bg-slate-100 dark:bg-slate-700 p-0.5 rounded-full border border-slate-200 dark:border-slate-600 text-xs text-black">
                  <button className={getOptionBtnClass(currentLang === 'VI')} onClick={() => i18n.changeLanguage('vi')} type="button">
                    VI
                  </button>
                  <button className={getOptionBtnClass(currentLang === 'EN')} onClick={() => i18n.changeLanguage('en')} type="button">
                    EN
                  </button>
                </div>
              </div>
              ...
              <button 
                className="w-full flex items-center justify-between px-4 py-2.5 text-sm text-black dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors cursor-pointer text-left"
              ...
              >
                ...
                <div className="flex items-center bg-slate-100 dark:bg-slate-700 p-0.5 rounded-full border border-slate-200 dark:border-slate-600 text-xs text-black pointer-events-none">
                  <div className={getOptionBtnClass(theme === 'light')}>
                    {t('header.theme_light')}
                  </div>
                  <div className={getOptionBtnClass(theme === 'dark')}>
                    {t('header.theme_dark')}
                  </div>
                </div>
              </button>
